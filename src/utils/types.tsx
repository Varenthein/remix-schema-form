...
export type FormFieldSchema<TypeName extends string, LangKey extends string, AdditionalSettings extends {
  options?: FormFieldOptions
} | void = void> = AdditionalSettings extends {
  options?: FormFieldOptions
} 
? {
  label: LangKey
...
export type AnyFormFieldsSchema = Record<string, FormFieldSchema<string, string> & { options?: any }>

// Type that supports all basic form fields schemas
// It can support also custom fields (if they present as type variable)
export type BaseFormFieldSchema<LangKey extends string, CustomFormFieldSchema = void> =
  BasicFieldsSchemas<LangKey> |
  (CustomFormFieldSchema extends FormFieldSchema<string, LangKey> ? CustomFormFieldSchema : never)
...
> = Required<Extract<BaseFormFieldSchema<LangKey, CustomFormFieldSchema>, {
  type: T,
  options?: unknown
}>>["options"]
...
export type OptionsForBasicType<
  ...
  LangKey extends string = string
> = Required<Extract<BaseFormFieldSchema<LangKey>, {
  type: T,
  options?: unknown
}>>["options"]
...
export type ValidatedFormBaseProps<Schema = BaseFormFieldsSchema<string, void>> = {
  schema: Schema
...
export type ValidatedFormBaseComponents = {
  ...
  message: ({ variant, items, children }: { variant: "success", items?: undefined, children: React.ReactNode } | { variant: "error", items: string[], children?: React.ReactNode }) => React.ReactNode
  submitBtn: ({ type, isSubmitting, children }: { type: "button" | "submit" | "reset", isSubmitting?: boolean, children: React.ReactNode }) => React.ReactNode
}
...
> = keyof ObjWithTypeKeys extends string
    ? ValidatedFormBaseComponents & Record<keyof ObjWithTypeKeys, FormFieldComponent<keyof ObjWithTypeKeys, CustomFormFieldSchema>>
...
export type FormFieldsSchemas<CustomSupportedFieldType extends string, LangKey extends string, T extends {
  [X in CustomSupportedFieldType]: {
    options?: FormFieldOptions
  }
} & Record<Exclude<keyof T, CustomSupportedFieldType>, never>> = T extends {
  [x in CustomSupportedFieldType]: {
    options?: FormFieldOptions
  }
} ? {
...
export type SchemaToZodObj<
  Schema extends Record<string, { validation: z.ZodTypeAny, conditioned?: (data: object) => Boolean }>
> = z.ZodObject<{
  [K in keyof Schema]: Schema[K]["conditioned"] extends object ? Schema[K]["validation"] | z.ZodUndefined : Schema[K]["validation"]
}>

// Type for custom getFormData function
export type GetFormDataFunc = <Schema extends AnyFormFieldsSchema, Conditionals extends AnySchemaConditionals>(
  request: Request,
  schema: Schema,
  schemaConditionals?: Conditionals
) => Promise<{
  data: { [k in keyof z.objectUtil.addQuestionMarks<z.baseObjectOutputType<{ [K in keyof Schema]: K extends keyof Conditionals ? Schema[K]["validation"] | z.ZodUndefined : Schema[K]["validation"]; }>, any>]: z.objectUtil.addQuestionMarks<z.baseObjectOutputType<{ [K in keyof Schema]: K extends keyof Conditionals ? Schema[K]["validation"] | z.ZodUndefined : Schema[K]["validation"]; }>, any>[k]; }
  cleanUp: (data: Record<string, any>) => Promise<void>,
  error: null,
  createCustomError: (errors: Partial<Record<keyof Schema, string> & { root: string | string[] }>) => Promise<TypedResponse<{
    errors: FieldErrors<{ [k in keyof z.objectUtil.addQuestionMarks<z.baseObjectOutputType<{ [K in keyof Schema]: K extends keyof Conditionals ? Schema[K]["validation"] | z.ZodUndefined : Schema[K]["validation"]; }>, any>]: z.objectUtil.addQuestionMarks<z.baseObjectOutputType<{ [K in keyof Schema]: K extends keyof Conditionals ? Schema[K]["validation"] | z.ZodUndefined : Schema[K]["validation"]; }>, any>[k]; }> | undefined
...
export type OptionsTranslationConfigBase = {
  [X in Required<Extract<BaseFormFieldSchema<string>, { options?: unknown }>> as X["type"]]?: {
...
export type OptionsTranslationConfigCustom<
  ...
  CustomFormFieldSchema extends FormFieldSchema<CustomType, string> & { options?: unknown }
> = {
  [X in Required<Extract<CustomFormFieldSchema, { options?: unknown }>> as X["type"]]?: RecursiveOption<X["options"]>
}
...
export type TranslateFunc = (key: string, values?: Record<string, any>) => string
...
export type ValidatedFormFieldsComponentsObj<Schema extends AnyFormFieldsSchema> = {
  [X in keyof Schema]: (props: { className?: string }) => React.ReactNode
}
export type ValidatedFormCompChildren<Schema extends AnyFormFieldsSchema> = (items: ValidatedFormFieldsComponentsObj<Schema>) => React.ReactNode
...
export type FormFieldsChildrenFunc<Schema extends AnyFormFieldsSchema> = (data: {
  Form: { 
...
export type SchemaConditionals<Schema extends AnyFormFieldsSchema> = Partial<
  {
    [K in keyof Schema]: (data: { [X in keyof Schema]: z.infer<Schema[X]["validation"]> | undefined }) => Boolean 
  } & { __watch: (keyof Schema)[] }>

export type AnySchemaConditionals = Record<string, ((data: any) => Boolean) | string[]> & { __watch?: string[] }

export type SchemaWithOptionalConditionals<Schema extends AnyFormFieldsSchema> = {
  [X in keyof Schema]: Schema[X] & { conditioned?: (data: object) => Boolean }
} 